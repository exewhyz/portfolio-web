...
const AboutTab = async () => {
  ...
  const certifications = await getCertificates();
    ...
    <TabsContent value="about" className="space-y-8 py-6">
      <Card className="overflow-hidden border-opacity-50 hover:shadow-md transition-all duration-300">
        ...
        <CardContent className="pt-6">
          <p className="text-lg leading-relaxed">{professionalData.about}</p>
        </CardContent>
      </Card>
        ...
        <Card className="overflow-hidden border-opacity-50 hover:shadow-md transition-all duration-300">
          ...
          <CardContent className="pt-4">
            <ul className="space-y-4 divide-y divide-border">
              {certifications?.map(
                (
                  cert: { title: string; institution: string; year: number },
                  index: number
                ) => (
                  <li
                    key={index}
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center text-sm text-muted-foreground mt-2 pl-6">
                      ...
                      <Badge
                        variant="outline"
                        className="rounded-full bg-accent/20 border-accent/30"
                      >
                        {cert.year}
                      </Badge>
                    </div>
